...
his-and-hers can notify you when a task completes — via Discord, Slack, or any
generic HTTP webhook. There are two levels of notification:

| Level | How | Fires on |
|-------|-----|----------|
| **Per-send** | `--notify <url>` flag | That invocation only |
| **Persistent** | `hh notify add <url>` | Every task, forever |
...
hh notify add https://discord.com/api/webhooks/1234/abc...

# Now every hh send --wait fires it automatically — no --notify needed
hh send "refactor auth.ts" --wait
```
...
```bash
hh notify add <url> [--name <label>] [--events all|complete|failure]
```

| Option | Default | Description |
|--------|---------|-------------|
| `--name <label>` | _(none)_ | Human-readable label, shown in `hh notify list` |
...
hh notify add https://hooks.slack.com/services/X/Y/Z \
  --name "slack-ops" \
  --events failure

# Generic endpoint — successes only
hh notify add https://myapp.com/hooks/hh \
  --name "app-api" \
  --events complete
```
...
hh notify test https://discord.com/api/webhooks/1234/abc
```
...
Webhooks are stored in `~/.his-and-hers/notify-webhooks.json`. The file is
...
Persistent webhooks fire automatically when `hh send --wait` receives a result.
...
- Check that `hh send` is run with `--wait`. Fire-and-forget sends (`hh send`
  without `--wait`) don't wait for a result so notifications never fire.
- Confirm webhooks are registered: `hh notify list`
- Test the URL is reachable: `hh notify test <id>`

**Discord: "Invalid Webhook Token"**

The webhook URL is stale. Delete the webhook in Discord server settings and
re-create it, then `hh notify remove` the old one and `hh notify add` the new URL.
