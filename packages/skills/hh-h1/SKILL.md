...
3. **Does H2 need waking?** If no heartbeat and WOL is enabled, run `hh wake` first.
...
### Handling results
- H2 will reply with a `HHMessage` of type `result`
...
- Wake command: `hh wake`
- Status check: `hh status`
