# cofounder-h2 — H2 (Executor) Skill
...
## Receiving work
- Incoming tasks arrive as `CofounderMessage` with type `task` or `handoff`
...
3. If blocked, send a `CofounderMessage` with type `error` explaining the blocker
4. On completion, send `CofounderMessage` with type `result` and `done: true`
