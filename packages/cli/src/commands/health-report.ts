/**
 * commands/health-report.ts — `hh health-report`
 *
 * Generate a comprehensive weekly health digest for a his-and-hers node pair.
 ...
 *   hh health-report --out report.md       # write to file
 *   hh health-report --json                # raw JSON data
 *   hh health-report --webhook <url>       # POST report to webhook
 *   hh health-report --no-verify-audit     # skip audit chain integrity check
 */
...
export function renderHealthReport(report: HealthReport): string {
  ...
  lines.push(`# 🏥 his-and-hers Health Report`);
  ...
  lines.push(`---`);
  lines.push(`\n_Generated by [his-and-hers](https://github.com/CalciferFriend/his-and-hers) · \`hh health-report\`_`);
...
export async function healthReport(opts: HealthReportOptions) {
  if (!opts.json) {
    p.intro(pc.bgGreen(pc.black(" his-and-hers health-report ")));
  }
