 ...
 * Usage:
 *   tj doctor
 *   tj doctor --peer glados   # focus on one peer
 *   tj doctor --json          # machine-readable output
 */
  ...
  checks.push(
    makeCheck(
      ...
      reachable ? undefined : wolEnabled
        ? "Machine may be sleeping — run: tj wake"
  ...
  if (!peerCaps) {
    ...
    checks.push(
      makeCheck(
        ...
        stale ? "Run: tj capabilities fetch" : undefined,
      ),
    );
  }
}
