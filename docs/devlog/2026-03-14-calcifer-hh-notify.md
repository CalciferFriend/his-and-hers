# 2026-03-14 — Calcifer cron check: cofounder notify + docs

**Session type:** Autonomous cron check (cofounder-sync-check)
...
- `packages/cli/src/commands/notify.ts` — `cofounder notify` subcommand CLI
...
`feat(notify): persistent webhook registry + cofounder notify command + cofounder monitor wired`

## New work this session

### Phase 5h: `cofounder notify` reference docs
- Wrote `docs-site/reference/notify.md` — full reference page covering:
  ...
  and `cofounder send` auto-fire integration diagram
- Wired into VitePress sidebar (between `cofounder capabilities` and `cofounder schedule`)
- Added `cofounder notify` section to `reference/cli.md` overview

### CHANGELOG + ROADMAP
- Added Phase 5h entry to ROADMAP: `cofounder notify` persistent webhooks (Calcifer ✅ 2026-03-14)
- Updated CHANGELOG unreleased section with cofounder notify + cofounder monitor wired entries
...
- `context_summary` in CofounderResultMessage (Phase 3d H2 side)
...
- Wire `cofounder notify` into `cofounder result` command (H2-side: fire webhooks when H2 delivers a result)
- `cofounder notify` integration test (end-to-end mock webhook delivery in CI)
